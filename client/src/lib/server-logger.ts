...
function emit(level: Level, event: string, ctx: Ctx): void {
  ...
  if (isProd) {
    console.log(JSON.stringify({ ts, level, event, source: "next", ...(ctx ?? {}) }));
    return;
  }
  const color = COLORS[level];
  const tag = `${color}${level.toUpperCase().padEnd(5)}${RESET}`;
  const ctxStr = ctx ? " " + DIM + JSON.stringify(ctx) + RESET : "";
  console.log(`${DIM}${ts}${RESET} ${tag} ${DIM}[next]${RESET} ${event}${ctxStr}`);
}
